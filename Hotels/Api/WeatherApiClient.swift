...
class WeatherApiClient {
    ...
    class func getCurrentWeatherByCoordinate(q: String, lat: Double, lon: Double, completion: @escaping (WeatherResponse?, Error?) ->  Void) {
        ...
        if let url  = URL(string: "https://community-open-weather-map.p.rapidapi.com/weather?q=\(q.trimmingCharacters(in: .whitespacesAndNewlines))&lat=\(lat)&lon=\(lon)&lang=null&units=metric") {
            var request = URLRequest(url: url, cachePolicy: .useProtocolCachePolicy)
            /*var request = try URLRequest(url: URL(string: )! as URL,
